...
def write_graph(
    ...
    if source_filter:
        ...
        ns.write_node_set(os.path.join(
            resource_path, f'nodes_{source_name}.tsv'
        ))
        es.write_edge_set(
            os.path.join(
                resource_path, f'edges_{source_name}.tsv'
            )
        )
    elif mixed:    
        ns, es =  get_graph_for_source(
        ...
        ns.write_node_set(
            os.path.join(resource_path, 'nodes_mixed.tsv')
        )
        es.write_edge_set(
            os.path.join(resource_path, 'edges_mixed.tsv')
        )
...
def get_graph_for_source(node_set: NodeSet, edge_set:EdgeSet, source_name:str = None, strict:bool = True, mixed:bool = False):
...
def write_artifacts(node_set:NodeSet, edge_set:EdgeSet, resource_types:list= RESOURCE_TYPES, resource_path:str = RESOURCE_PATH):
...
def merge_resource_sets(resource_path:str = RESOURCE_PATH, write_resource:bool = True):

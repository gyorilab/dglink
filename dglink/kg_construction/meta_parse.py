...
def get_entities_from_meta(
    study_metadata, ground_fields, unground_fields, nodes: NodeSet
):
    ...
    for field in ground_fields + unground_fields:
        if field in study_metadata.keys():
            ...
            for entry in field_val:
                ...
                ## dont want to double up on the related studies
                if field != "relatedStudies":
                    working_node = Node(
                        attribute_names=ENTITY_ATTRIBUTES, attributes=node_attributes
                    )
...
def get_entities_from_wiki(
    study_wiki, wiki_fields, entity_nodes: NodeSet, project_nodes: NodeSet
):
    ...
    project_node = Node(
        ...
        attributes={"curie:ID": wiki_id, ":LABEL": "Wiki"},
    )
